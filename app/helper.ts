...
export async function UploadFile(file: { move: (arg0: string, arg1: { name: string; overwrite: boolean }) => any; extname: any }, namefile: string, pathtarget: string) {
...
export function DateTimeFormated(format: string | undefined, date: moment.MomentInput) {
...
export function uniqueDatime(xDate: { getFullYear: () => { (): any; new(): any; toString: { (arg0: number): string; new(): any } }; getMonth: () => number; getDate: () => { (): any; new(): any; toString: { (arg0: number): string; new(): any } }; getHours: () => { (): any; new(): any; toString: { (arg0: number): string; new(): any } }; getMinutes: () => { (): any; new(): any; toString: { (arg0: number): string; new(): any } }; getSeconds: () => { (): any; new(): any; toString: { (arg0: number): string; new(): any } } }) {
...
export function response(code: number, msg: any) {
    switch (code) {
        case 200:
            return { res: { status: true, data: msg, msg: 'success' }, statCode: code }
        case 400:
            return { res: { status: false, data: msg, msg: 'error' }, statCode: code }
        case 401:
            return { res: { status: false, data: msg, msg: 'error' }, statCode: code }
        case 422:
            return { res: { status: false, data: msg, msg: 'error' }, statCode: code }
...
export function ParamsGeaApiServices() {
    return {
        "id": "Wiliang",
        "key": "thekeyhereisjustfortestingwillbechangedlater"
    }
}
...
export async function FindUserAuthLocalDb(username: string, password: string) {
    try {
        const findUser = await User.query().where(async (query) => {
            query
        ...
        if (isSame) {
            ...
            update.islogin = 'y'
            await update.save()
            return response(200, { username: findUser!.username, password: password })
        } else {
